...
class DynamicTFPolicy(TFPolicy):
    ...
    def __init__(self,
        ...
        # neighbor_obs_dic = {}
        neighbor_obs = None
        if existing_inputs is not None:  # None
            obs = existing_inputs[SampleBatch.CUR_OBS]
            neighbor_obs = existing_inputs['neighbor_obs']
            ...
            '''For Attention'''
            # # (dict, Tensor): (4, None, 15)
            # for k in range(4):
            #     neighbor_obs_dic['neighbor_obs_{}'.format(k)] = tf.placeholder(
            #         tf.float32,
            #         shape=[None] + list(obs_space.shape),
            #         name='neighbor_obs_{}'.format(k))

            # Tensor: (None, 5, 15)
            neighbor_obs = tf.placeholder(
                tf.float32,
                shape=[None, 5] + list(obs_space.shape),
                name="neighbor_obs")
            # -----------------------------------------------------
        ...
        model_out, self.state_out = self.model(
        ...
        TFPolicy.__init__(
            ...
            obs_input=obs,
            # For Attention
            neighbor_obs_input=neighbor_obs,
            # -----------------------------
    ...
    def _initialize_loss(self):
        ...
        for i, si in enumerate(self.state_in):
            train_batch["state_in_{}".format(i)] = si
        train_batch["seq_lens"] = self.seq_lens

        '''For Attention'''
        # for k in range(4):
        #     batch_tensors['neighbor_obs_{}'.format(k)] = tf.placeholder(np.float32, shape=(None, 15),
        #                                                                 name='neighbor_obs_{}'.format(k))
        train_batch['neighbor_obs'] = tf.placeholder(np.float32, shape=(None, 5, 29), name='neighbor_obs')
