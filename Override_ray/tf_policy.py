...
from Override_ray.tf_run_builder import TFRunBuilder
...
class TFPolicy(Policy):
    ...
    def __init__(self,
        ...
        # For Attention
        self._neighbor_obs_input = neighbor_obs_input
        # ---------------------
    ...
    def compute_gradients(self, postprocessed_batch, neighbor_batch_dic):
        ...
        fetches = self._build_compute_gradients(builder, postprocessed_batch, neighbor_batch_dic)
        return builder.get(fetches)
    ...
    def learn_on_batch(self, postprocessed_batch, neighbor_batch_dic):
        ...
        fetches = self._build_learn_on_batch(builder, postprocessed_batch, neighbor_batch_dic)
        return builder.get(fetches)
    ...
    def _build_signature_def(self):
        ...
        '''For Attention'''
        input_signature["neighbor_obs"] = \
            tf.saved_model.utils.build_tensor_info(self._neighbor_obs_input)
        # ---------------------------------------------------------------
    ...
    def _build_compute_actions(self,
        ...
        '''For Attention'''
        builder.add_feed_dict({self._neighbor_obs_input: neighbor_obs_batch})
        # ---------------------------------------------
        if state_batches:
    ...
    def _build_compute_gradients(self, builder, postprocessed_batch, neighbor_batch_dic):
        self._debug_vars()
        builder.add_feed_dict(self.extra_compute_grad_feed_dict())
        builder.add_feed_dict({self._is_training: True})
        builder.add_feed_dict(
            self._get_loss_inputs_dict(postprocessed_batch, neighbor_batch_dic, shuffle=False))
    ...
    def _build_learn_on_batch(self, builder, postprocessed_batch, neighbor_batch_dic):
        self._debug_vars()
        builder.add_feed_dict(self.extra_compute_grad_feed_dict())
        builder.add_feed_dict(
            self._get_loss_inputs_dict(postprocessed_batch, neighbor_batch_dic, shuffle=False))
            ...
            for k, ph in self._loss_inputs:
                ...
                if 'neighbor_obs' in k:
                    ...
                    for neighbor_id in neighbor_list:
                        tmp_dic[ph][neighbor_id] = neighbor_batch_dic[neighbor_id]['obs']
                    ...
                    for batch_item in range(len(tmp_dic[ph][neighbor_id])):
                        feed_dict[ph].append([])
                        for neighbor_id in neighbor_list:
                            feed_dict[ph][batch_item].append(tmp_dic[ph][neighbor_id][batch_item])
