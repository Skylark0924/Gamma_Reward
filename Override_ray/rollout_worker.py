...
from Override_ray.sampler import AsyncSampler, SyncSampler
        ...
        if isinstance(samples, MultiAgentBatch):
            ...
            if self.tf_sess is not None:
                ...
                for pid, batch in samples.policy_batches.items():
                    ...
                    '''
                    neighbor_batch_dic = {}
                    neighbor_pid_list = [pid_ for pid_ in
                                         self.traffic_light_node_dict[self.inter_num_2_id(int(pid.split('_')[1]))][
                                             'adjacency_row'] if pid_ != None]
                    for neighbor_pid in neighbor_pid_list:
                        neighbor_batch_dic['policy_{}'.format(neighbor_pid)] = samples.policy_batches[
                ...
                for pid, batch in samples.policy_batches.items():
                    ...
                    # ------------------------------------------------------------------
                    grad_out[pid], info_out[pid] = (
                        self.policy_map[pid].compute_gradients(batch, neighbor_batch_dic))
        else:
            grad_out, info_out = (
        ...
        if isinstance(samples, MultiAgentBatch):
            ...
            for pid, batch in samples.policy_batches.items():
                ...
                if builder and hasattr(policy, "_build_learn_on_batch"):
                    ...
                    info_out[pid] = policy.learn_on_batch(batch, neighbor_batch_dic)
