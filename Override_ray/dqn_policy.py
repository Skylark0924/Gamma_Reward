...
from Model.distributional_q_model import DistributionalQModel
...
from Override_ray.tf_policy import LearningRateSchedule
from Override_ray.tf_policy_template import build_tf_policy
...
class ComputeTDErrorMixin(object):
    def __init__(self):
        ...
        def compute_td_error(obs_t, neighbor_t, act_t, rew_t, obs_tp1, done_mask,
                             importance_weights):
            # Do forward pass on loss to update td error attribute
            build_q_losses(
                self, self.model, None, {
                    SampleBatch.CUR_OBS: tf.convert_to_tensor(obs_t),
                    'neighbor_obs': tf.convert_to_tensor(neighbor_t),
...
def build_q_networks(policy, q_model, input_dict, obs_space, action_space,
    ...
    q_values, q_logits, q_dist = _compute_q_values(
        policy, q_model, input_dict[SampleBatch.CUR_OBS], input_dict['neighbor_obs'], obs_space,
        action_space)
...
def build_q_losses(policy, model, _, train_batch):
    ...
    q_t, q_logits_t, q_dist_t = _compute_q_values(
        policy, policy.q_model, train_batch[SampleBatch.CUR_OBS], train_batch['neighbor_obs'],
        policy.observation_space, policy.action_space)

    # target q network evalution
    q_tp1, q_logits_tp1, q_dist_tp1 = _compute_q_values(
        policy, policy.target_q_model, train_batch[SampleBatch.NEXT_OBS], train_batch['neighbor_obs'],
        policy.observation_space, policy.action_space)
    ...
    # for k in range(4):
    #     nothing = batch_tensors['neighbor_obs_{}'.format(k)]
    # neighbor_obs = batch_tensors['neighbor_obs']
...
def _compute_q_values(policy, model, obs, neighbor_obs, obs_space, action_space):
    config = policy.config
...
def _postprocess_dqn(policy, batch):
    ...
    if batch.count > 0 and policy.config["worker_side_prioritization"]:
        td_errors = policy.compute_td_error(
            batch[SampleBatch.CUR_OBS], batch['neighbor_obs'], batch[SampleBatch.ACTIONS],
            batch[SampleBatch.REWARDS], batch[SampleBatch.NEXT_OBS],
...
DQNTFPolicy = build_tf_policy(
    name="DQNTFPolicy",
    get_default_config=lambda: dqn.DEFAULT_CONFIG,
