...
from Override_ray.tf_policy import TFPolicy
...
PolicyEvalData = namedtuple("PolicyEvalData", [
    "env_id", "agent_id", "obs", "neighbor_obs", "info", "rnn_state", "prev_action",
    "prev_reward"
])

i = 0
tmp_dic = None
traffic_light_node_dict = None
...
def _process_observations(base_env, policies, batch_builder_pool,
    ...
    if i <= 1:
        ...
        tmp_dic = traffic_light_node_dict['intersection_1_1']['inter_id_to_index']
    ...
    for env_id, agent_obs in unfiltered_obs.items():
        ...
        for agent_id, raw_obs in agent_obs.items():
            ...
            neighbor_pid_list = ['policy_{}'.format(pid_) for pid_ in
                                 traffic_light_node_dict[inter_num_2_id(int(policy_id.split('_')[1]))][
                                     'adjacency_row'] if pid_ != None]
            ...
            for neighbor_id in neighbor_pid_list:
                neighbor_prep_obs = _get_or_raise(preprocessors,
                                                  neighbor_id).transform(raw_obs)
                neighbor_filtered_obs = _get_or_raise(obs_filters,
                                                      neighbor_id)(neighbor_prep_obs)
                neighbor_obs[0].append(neighbor_filtered_obs)
                i += 1
            neighbor_obs = np.array(neighbor_obs).reshape((len(neighbor_pid_list), len(raw_obs)))  # (5, 29)
